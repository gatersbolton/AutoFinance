...
class WebAppTests(unittest.TestCase):
    ...
    def test_document_step2_standard_mapping_after_raw_metrics_exists(self):
        ...
        self.assertIn("data-result-primary-actions", continue_response.text)
        self.assertIn("data-result-downloads", continue_response.text)
        self.assertIn(">数据表 Excel</a>", continue_response.text)
        self.assertIn(">数据表 CSV</a>", continue_response.text)
        self.assertIn(">预览数据表</a>", continue_response.text)
        self.assertIn("更多下载", continue_response.text)
        self.assertIn("重新处理数据", continue_response.text)
        ...
        accounting_response = self.client.post(f"/documents/{doc_id}/generate-accounting-workbook")
        self.assertEqual(accounting_response.status_code, 200)
        self.assertTrue(accounting_response.content.startswith(b"PK"))
        accounting_ready_page = self.client.get(f"/documents/{doc_id}/continue")
        self.assertIn(">会计报表 Excel</a>", accounting_ready_page.text)
        self.assertIn(">重新生成并下载会计报表</button>", accounting_ready_page.text)
        self.assertNotIn(">生成并下载会计报表</button>", accounting_ready_page.text)
    ...
    def test_unified_proofread_page_loads_combined_table_and_saves_edits(self):
        ...
        self.assertIn("/static/app.js?v=current-review-20260820-1", response.text)
        self.assertIn("/static/style.css?v=current-review-20260820-1", response.text)
        ...
        self.assertIn('placeholder="搜索标准术语"', response.text)
        self.assertNotIn("如：短期、借款、68、dqjk", response.text)
    ...
    def test_unified_proofread_source_attrs_confidence_missing_and_autocomplete_bug_contract(self):
        ...
        self.assertIn(".unified-col-period-value {\n    width: 128px;", style)
        self.assertIn(".unified-col-standard-term {\n    width: 160px;", style)
